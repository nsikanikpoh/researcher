...
from tools import ingest_financial_document, update_todo
...
async def run_research_agent(topic: str = None) -> str:
    ...
    if topic:
        query = (
            ...
            f"Begin by copying your TODO LIST from instructions, "
            f"then work through each step in order."
        )
    else:
    ...
    with trace("Researcher"):
        async with create_playwright_mcp_server(timeout_seconds=60) as playwright_mcp:
            agent = Agent(
                ...
                model=model,
                tools=[ingest_financial_document, update_todo],
